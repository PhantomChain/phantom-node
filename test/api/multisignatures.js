...
function sendPhantom (account, i, done) {
...
function sendPhantomFromMultisigAccount (amount, recipient, done) {
	node.put('/api/transactions/', {
...
before(function (done) {
	...
	async.eachSeries(accounts, function (account, eachCb) {
		sendPhantom(account, i, function () {
			i++;
			return eachCb();
		});
	}, function (err) {
...
before(function (done) {
	sendPhantom(multisigAccount, null, done);
});
...
describe('PUT /api/api/transactions/', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendPhantomFromMultisigAccount(100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
